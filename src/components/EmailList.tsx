...
function QuoteDivider({ quote }: { quote?: { quote: string; author: string } }) {
  return (
    <div className="flex items-center gap-3 px-3 py-2 border-b border-gray-100 bg-gradient-to-r from-indigo-50/60 to-purple-50/60 select-none">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="text-xs text-purple-300">inspiration</span>
        </div>
    ...
    <div className="flex flex-col">
      ...
      {emails.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-64 gap-3 text-gray-400">
          ...
          <p className="text-sm">
            {isSearch ? 'No results found.' : `${title} is empty.`}
          </p>
        </div>
      ) : (
        <>
          {emails.map((email, i) => (
            <div key={email.id}>
              {i > 0 && (
                <QuoteDivider
                  quote={quotes ? quotes[(i - 1) % quotes.length] : undefined}
                />
              )}
